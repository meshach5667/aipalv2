...
class MemoryControlCenterScreen extends StatefulWidget {
  ...
  @override
  State<MemoryControlCenterScreen> createState() => _MemoryControlCenterScreenState();
}
...
class _MemoryControlCenterScreenState extends State<MemoryControlCenterScreen> {
  ...
  Future<List<dynamic>> _loadMemories() => context.read<AppState>().api.listMemories();

  Future<List<dynamic>> _loadPendingMemories() => context.read<AppState>().api.listPendingMemories();
  ...
  Future<void> _exportMemories() async {
    ...
    if (!mounted) return;
    final filename = 'aipal-memories-${DateTime.now().toIso8601String().split('T').first}.json';
  ...
  Future<void> _clearHistory() async {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text('This removes the saved chat history for the account.'),
        actions: [
  ...
  Future<void> _editMemory([Map<String, dynamic>? memory]) async {
    final api = context.read<AppState>().api;
    final titleController = TextEditingController(text: memory?['title']?.toString() ?? '');
    final contentController = TextEditingController(text: memory?['content']?.toString() ?? '');
    final typeController = TextEditingController(text: memory?['type']?.toString() ?? 'fact');
    final lifeAreaController = TextEditingController(text: memory?['life_area']?.toString() ?? '');
    final importanceController = TextEditingController(text: memory?['importance']?.toString() ?? '1');
    final confidenceController = TextEditingController(text: memory?['confidence']?.toString() ?? '0.5');
    final expiresAtController = TextEditingController(text: memory?['expires_at']?.toString() ?? '');
    final reasonController = TextEditingController(text: memory?['suggested_reason']?.toString() ?? '');
    ...
    final result = await showModalBottomSheet<Map<String, dynamic>>(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (context, setSheetState) => _MemoryEditorSheet(
          ...
          onApprovalStatusChanged: (v) => setSheetState(() => approvalStatus = v),
          ...
          child: Column(
            children: [
              Padding(
                ...
                child: Row(
                  children: [
                    _HeaderButton(icon: Icons.arrow_back_rounded, onTap: () => Navigator.pop(context)),
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            ...
                            style: TextStyle(fontSize: 13, color: Color(0xFF575C6B)),
                          ),
                        ],
                      ),
                    ),
              ...
              const Padding(
                ...
                child: TabBar(
                  labelColor: Color(0xFF6F5081),
                  unselectedLabelColor: Color(0xFF6E6A71),
                  indicatorColor: Color(0xFF6F5081),
                  tabs: [
                    ...
                    children: [
                      ...
                      FutureBuilder<List<dynamic>>(
                        ...
                        builder: (context, snapshot) {
                          ...
                          return _MemoryList(
                            memories: memories,
                            emptyState: const _EmptyState(text: 'No pending memories right now.'),
...
class _MemoryCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final approvalStatus = memory['approval_status']?.toString() ?? (approved ? 'approved' : 'pending');
        ...
        children: [
          Row(
            children: [
              ...
              if (paused)
                const _StatusChip(label: 'Paused', color: Color(0xFF326667))
              else
                const _StatusChip(label: 'Active', color: Color(0xFF6F5081)),
              _StatusChip(
          ...
          Wrap(
            ...
            runSpacing: 8,
            children: [
              _StatusChip(label: memory['type']?.toString() ?? 'fact', color: const Color(0xFF1B1C1A)),
              if (memory['life_area'] != null && memory['life_area'].toString().isNotEmpty)
                _StatusChip(label: memory['life_area'].toString(), color: const Color(0xFF326667)),
              if (sensitive)
                const _StatusChip(label: 'Sensitive', color: Color(0xFFBA1A1A)),
              if (!approved)
                const _StatusChip(label: 'Needs approval', color: Color(0xFFBA1A1A)),
            ],
          ),
          const SizedBox(height: 10),
          Text(
            memory['content']?.toString() ?? '',
            style: const TextStyle(fontSize: 13.5, height: 1.5, color: Color(0xFF4B444D)),
          ),
          if (suggestedReason != null && suggestedReason.isNotEmpty) ...[
            const SizedBox(height: 8),
            Text(
              suggestedReason,
              style: const TextStyle(fontSize: 12.5, height: 1.4, color: Color(0xFF575C6B)),
            ),
          ],
          if (expiresAt != null && expiresAt.isNotEmpty) ...[
            const SizedBox(height: 8),
            Text(
              memoryScope == 'temporary' ? 'Expires: $expiresAt' : 'Expires: $expiresAt',
              style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: Color(0xFF326667)),
            ),
          ],
          ...
          Wrap(
            ...
            children: [
              ...
              OutlinedButton(onPressed: onPause, child: Text(paused ? 'Resume' : 'Pause')),
              if (!showPendingActions)
                FilledButton.tonal(onPressed: onDelete, child: const Text('Delete')),
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            _Field(controller: contentController, label: 'Content', maxLines: 4),
            const SizedBox(height: 12),
            Row(
              children: [
                Expanded(child: _Field(controller: typeController, label: 'Type')),
                const SizedBox(width: 12),
                Expanded(child: _Field(controller: lifeAreaController, label: 'Life area')),
              ],
            ),
            const SizedBox(height: 12),
            Row(
              children: [
                Expanded(child: _Field(controller: importanceController, label: 'Importance')),
                const SizedBox(width: 12),
                Expanded(child: _Field(controller: confidenceController, label: 'Confidence')),
              ],
            ),
            const SizedBox(height: 10),
            Row(
              children: [
                ...
                const SizedBox(width: 12),
                Expanded(child: _Field(controller: expiresAtController, label: 'Expires at (ISO)')),
              ],
            ),
            const SizedBox(height: 12),
            _Field(controller: reasonController, label: 'Suggested reason', maxLines: 2),
            const SizedBox(height: 12),
                ...
                onPressed: () {
                  Navigator.pop(
                    context,
                    {
                      ...
                      'importance': int.tryParse(importanceController.text.trim()) ?? 1,
                      'confidence': double.tryParse(confidenceController.text.trim()) ?? 0.5,
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Text(
        ...
        style: const TextStyle(fontWeight: FontWeight.w700, color: Color(0xFF1B1C1A)),
      ),
    );
  }
}
...
class _EmptyState extends StatelessWidget {
  const _EmptyState({this.text = 'No memories saved yet. AiPal will start collecting useful context as you talk.'});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Text(
        text,
        style: const TextStyle(fontSize: 14, height: 1.5, color: Color(0xFF4B444D)),
      ),
    );
  }
}
