...
class _CommitmentsScreenState extends State<CommitmentsScreen> {
  ...
  Future<Map<String, List<dynamic>>> _load() async {
    ...
    final results = await Future.wait([api.getCommitments(), api.getDueCommitments()]);
    return {
      'commitments': results[0],
      'due': results[1],
    };
  }
          ...
          children: [
            ...
            FutureBuilder<Map<String, List<dynamic>>>(
              ...
              builder: (context, snapshot) {
                ...
                final open = commitments.where((item) => item['status'] == 'open').toList();
                final completed = commitments.where((item) => item['status'] == 'completed').toList();
                final dismissed = commitments.where((item) => item['status'] == 'dismissed').toList();
...
class _CommitmentSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return _GlassPanel(
      borderColor: highlighted ? _teal.withValues(alpha: 0.25) : Colors.white.withValues(alpha: 0.68),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      title,
                      style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: _purple),
                    ),
                    const SizedBox(height: 4),
                    Text(subtitle, style: const TextStyle(color: Color(0xFF706A73))),
                  ],
                ),
              ),
      ...
      child: Column(
        ...
        children: [
          ...
          if (!compact && content.isNotEmpty) ...[
            const SizedBox(height: 8),
            Text(content, style: const TextStyle(color: Color(0xFF5F5963), height: 1.35)),
          ],
          ...
          Wrap(
            ...
            runSpacing: 8,
            children: [
              if (dueLabel != null) _InfoPill(icon: Icons.event_rounded, text: dueLabel),
              if (followUpLabel != null) _InfoPill(icon: Icons.forum_rounded, text: followUpLabel),
              if (related != null && related.isNotEmpty) _InfoPill(icon: Icons.hub_rounded, text: related),
              if (confidence is num) _InfoPill(icon: Icons.verified_rounded, text: '${(confidence * 100).round()}%'),
            ],
          ),
...
class _MetricChip extends StatelessWidget {
  const _MetricChip({required this.label, required this.value, this.accent = _purple});
...
class _CountBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Text('$count', style: const TextStyle(color: _purple, fontWeight: FontWeight.w800)),
      );
}
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Row(
          ...
          children: [
            ...
            Text(text, style: const TextStyle(fontSize: 12, color: _ink, fontWeight: FontWeight.w600)),
          ],
        ),
      );
}
        ...
        child: BackdropFilter(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(26),
              border: Border.all(color: borderColor ?? Colors.white.withValues(alpha: 0.74)),
              boxShadow: [
                BoxShadow(
