...
class _GoalsScreenState extends State<GoalsScreen> {
  ...
  Future<List<dynamic>> _loadGoals() => context.read<AppState>().api.listGoals();
  ...
  Future<void> _editGoal([Map<String, dynamic>? goal]) async {
    ...
    final descriptionController = TextEditingController(text: goal?['description']?.toString() ?? '');
    final lifeAreaController = TextEditingController(text: goal?['life_area']?.toString() ?? '');
    final targetDateController = TextEditingController(text: goal?['target_date']?.toString().split('T').first ?? '');
                        ...
                        if (goals.isEmpty)
                          ...
                          ...goals.map(
                            (goal) => _GoalCard(
                              ...
                              onEdit: () => _editGoal(goal.cast<String, dynamic>()),
                              onDelete: () => _deleteGoal(goal['id'].toString()),
                            ),
                          ),
                      ],
                    );
            ...
            child: Column(
              ...
              children: [
                  ...
                  children: [
                    Expanded(
                      child: Text(
                        ...
                        style: const TextStyle(fontFamily: 'Manrope', fontSize: 18, fontWeight: FontWeight.w800, color: Color(0xFF1B1C1A)),
                      ),
                    ),
                ...
                if ((goal['description']?.toString() ?? '').isNotEmpty)
                  Text(goal['description'].toString(), style: const TextStyle(fontSize: 13.5, height: 1.5, color: Color(0xFF4B444D))),
                const SizedBox(height: 8),
                Wrap(
                  spacing: 8,
                  runSpacing: 8,
                  children: [
                    ...
                    if ((goal['target_date']?.toString() ?? '').isNotEmpty)
                      _Chip(label: goal['target_date'].toString().split('T').first),
                  ],
                ),
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            _Field(controller: descriptionController, label: 'Description', maxLines: 4),
            const SizedBox(height: 12),
            Row(
              children: [
                Expanded(child: _Field(controller: lifeAreaController, label: 'Life area')),
                const SizedBox(width: 12),
                Expanded(child: _Field(controller: targetDateController, label: 'Target date YYYY-MM-DD')),
              ],
            ),
            const SizedBox(height: 12),
            Row(
                ...
                onPressed: () {
                  Navigator.pop(
                    context,
                    {
                      ...
                      'target_date': targetDateController.text.trim().isEmpty ? null : targetDateController.text.trim(),
                      'status': localStatus,
...
class _EmptyState extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: const Text('No goals yet. Create one to start linking your work and intentions.', style: TextStyle(fontSize: 14, height: 1.5, color: Color(0xFF4B444D))),
    );
  }
}
