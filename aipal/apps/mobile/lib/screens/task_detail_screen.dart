...
class _TaskDetailScreenState extends State<TaskDetailScreen> {
  ...
  Future<void> _editTask(Map<String, dynamic> task) async {
    ...
    final notesController = TextEditingController(text: task['notes']?.toString() ?? '');
      ...
      body: SafeArea(
        child: FutureBuilder<Map<String, dynamic>>(
          ...
          builder: (context, snapshot) {
            ...
            final subtasks = (data?['subtasks'] as List?)?.cast<Map<String, dynamic>>() ?? [];
                            ...
                            child: Column(
                              ...
                              children: [
                                Wrap(
                                  ...
                                  runSpacing: 8,
                                  children: [
                                    _Chip(label: task?['status']?.toString() ?? 'planned'),
                                    _Chip(label: task?['priority']?.toString() ?? 'medium'),
                                    if ((task?['due_at']?.toString() ?? '').isNotEmpty)
                                      _Chip(label: _formatDate(task?['due_at']?.toString())),
                                    if ((goal?['title']?.toString() ?? '').isNotEmpty)
                                ...
                                Wrap(
                                  ...
                                  children: [
                                    FilledButton.icon(
                                      onPressed: task == null ? null : () => _toggleComplete(task),
                                      icon: const Icon(Icons.done_rounded, size: 18),
                                      label: Text(
                                        (task?['status']?.toString() ?? 'planned') == 'done'
                                            ? 'Mark open'
                                            : 'Mark done',
                                      ),
                                    ),
                                    OutlinedButton.icon(
                                      onPressed: task == null ? null : () => _editTask(task),
                                      icon: const Icon(Icons.edit_rounded, size: 18),
                                      label: const Text('Edit'),
                                    ),
                                    OutlinedButton.icon(
                                      onPressed: task == null ? null : () => _linkGoal(task),
                                      icon: Icon(
                                        goal == null ? Icons.flag_rounded : Icons.flag_outlined,
                                        size: 18,
                                      ),
                                      label: Text(goal == null ? 'Link goal' : 'Change goal'),
                                    ),
                                    FilledButton.tonalIcon(
                                      onPressed: task == null ? null : () => _deleteTask(task),
...
class _GoalOption extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Material(
        color: selected ? const Color(0xFFF4EAF7) : Colors.white.withValues(alpha: 0.68),
          ...
          child: Padding(
            ...
            child: Row(
              children: [
                ...
                if (selected)
                  const Icon(Icons.check_rounded, color: Color(0xFF6F5081), size: 18),
              ],
            ),
          ),
        ),
          ...
          child: Row(
            children: [
              Container(
                ...
                child: const Icon(Icons.flag_rounded, color: Color(0xFF6F5081), size: 20),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: Column(
