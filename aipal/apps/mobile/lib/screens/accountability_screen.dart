...
class _AccountabilityScreenState extends State<AccountabilityScreen> {
  ...
  Future<void> _generate() async {
    ...
    final start = now.subtract(const Duration(days: 6)).toIso8601String().split('T').first;
    final end = now.toIso8601String().split('T').first;
    final snapshot = await api.createAccountabilitySnapshot(periodStart: start, periodEnd: end);
    if (!mounted) return;
    setState(() {
      _snapshot = snapshot;
      _future = _load();
              ...
              children: [
                _GlassPanel(
                  child: Column(
                    ...
                    children: [
                      const Text('Weekly accountability', style: TextStyle(fontSize: 28, fontWeight: FontWeight.w800, color: _purple)),
                      const SizedBox(height: 8),
                      const Text('A lightweight check on progress, habits, blockers, and your next accountability question.'),
                ...
                if (_comparison != null) ...[
                  ...
                  _GlassPanel(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text('Comparison', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: _purple)),
                        const SizedBox(height: 8),
                        Text(_comparison!['accountability_question']?.toString() ?? ''),
                        const SizedBox(height: 8),
  ...
  Widget build(BuildContext context) {
    return _GlassPanel(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Latest snapshot', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: _purple)),
          const SizedBox(height: 8),
          Text(snapshotData['reflection']?.toString() ?? 'No reflection yet.'),
          const SizedBox(height: 8),
          Text('Score: ${snapshotData['score']?.toString() ?? '—'}', style: const TextStyle(color: _teal)),
          const SizedBox(height: 8),
