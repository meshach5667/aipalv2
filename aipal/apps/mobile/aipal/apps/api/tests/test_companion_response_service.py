...
async def test_generate_companion_response_caps_context_at_ten_items():
    ...
    assert len(result["context_items_used"]) <= 16
    memory_lines = [
        line for line in messages[1]["content"].splitlines()
        if line.startswith("- Qring memory")
    ]
    assert len(memory_lines) <= 16


@pytest.mark.asyncio
async def test_personalized_response_prompt_uses_stored_user_context():
    result, messages = await _call_service(
        "What should I do about Qring today?",
        memories=[
            {
                "title": "Qring estate pilot",
                "content": "User wants estate chairmen outreach before product polishing.",
                "approval_status": "approved",
                "user_approved": True,
                "importance": 9,
                "confidence": 0.9,
        ...
        tasks=[{"title": "Call estate chairmen", "status": "open", "confidence": 0.9}],
    )

    prompt = messages[1]["content"]
    assert "Qring estate pilot" in prompt
    assert "Call estate chairmen" in prompt
    assert "Grow Qring" in prompt
    assert result["reply"]


@pytest.mark.asyncio
async def test_follow_up_request_preserves_recent_conversation_history():
    _result, messages = await _call_service("What about the second one?")

    prompt = messages[1]["content"]
    assert "Recent turns:" in prompt
    assert "I have been working on Qring." in prompt


@pytest.mark.asyncio
async def test_direct_answer_prompt_discourages_unnecessary_clarification():
    _result, messages = await _call_service("Should I call Stephen now?")

    system = messages[0]["content"]
    assert "answer it directly before suggesting action" in system
    assert "Ask a clarifying question only when a required fact is missing" in system


@pytest.mark.asyncio
async def test_malformed_structured_provider_response_recovers_user_reply():
    result, _messages = await _call_service_with_reply(
        "Help me plan Qring outreach.",
        "reply: Start with the estate chairmen calls today.\nmode: companion",
    )

    assert result["reply"] == "Start with the estate chairmen calls today."


@pytest.mark.asyncio
async def test_no_hard_coded_generic_reply_when_model_succeeds():
    result, _messages = await _call_service_with_reply(
        "Help me with Qring.",
        "Use the Qring estate pilot context and call the estate chairmen first.",
    )

    assert "I’m here with you" not in result["reply"]
    assert "estate chairmen" in result["reply"]
