...
async def test_llm_chat_stream_deepseek_parses_sse():
    ...
    with patch("app.llm_provider.settings") as mock_settings:
        ...
        mock_settings.deepseek_max_tokens = 700
        mock_settings.deepseek_temperature = 0.25
        mock_settings.deepseek_timeout_seconds = 18
...
@pytest.mark.asyncio
async def test_llm_chat_deepseek_active_without_fallback_uses_configured_model():
    from app.llm_provider import active_provider_status, llm_chat
    ...
    class FakeResp:
        ...
        def json(self):
            return {"choices": [{"message": {"content": "DeepSeek hello"}}]}
    ...
    assert reply == "DeepSeek hello"
    assert captured["url"] == "https://api.deepseek.com/chat/completions"
    assert captured["kwargs"]["json"]["model"] == "deepseek-chat"
    assert captured["kwargs"]["json"]["max_tokens"] == 700


@pytest.mark.asyncio
async def test_llm_chat_deepseek_missing_key_does_not_silently_fallback():
    from app.llm_provider import active_provider_status, llm_chat
    ...
    assert status.fallback_reason == "missing_deepseek_api_key"
