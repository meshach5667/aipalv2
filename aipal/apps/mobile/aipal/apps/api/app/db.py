...
async def init_db() -> None:
    async with engine.begin() as conn:
        ...
        if conn.dialect.name == "sqlite":
            ...
            async def add_user_column(name: str, ddl: str) -> None:
                if name not in user_columns:
                    await conn.exec_driver_sql(f"ALTER TABLE users ADD COLUMN {ddl}")

            await add_user_column("display_name", "display_name VARCHAR(120) NULL")
            await add_user_column("wake_name", "wake_name VARCHAR(120) NULL")
            await add_user_column("timezone", "timezone VARCHAR(64) NOT NULL DEFAULT 'UTC'")
            await add_user_column("about_me", "about_me TEXT NULL")
            await add_user_column("morning_brief_at", "morning_brief_at TIME NULL")
            await add_user_column("evening_recap_at", "evening_recap_at TIME NULL")
            await add_user_column("checkin_enabled", "checkin_enabled BOOLEAN NOT NULL DEFAULT 1")
            await add_user_column(
                "knowledge_gap_data_consent",
                "knowledge_gap_data_consent BOOLEAN NOT NULL DEFAULT 0",
            )
            await add_user_column("updated_at", "updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP")
        if conn.dialect.name == "postgresql":
            ...
            await conn.exec_driver_sql("ALTER TABLE users ADD COLUMN IF NOT EXISTS display_name VARCHAR(120) NULL")
            await conn.exec_driver_sql("ALTER TABLE users ADD COLUMN IF NOT EXISTS wake_name VARCHAR(120) NULL")
            await conn.exec_driver_sql("ALTER TABLE users ADD COLUMN IF NOT EXISTS timezone VARCHAR(64) NOT NULL DEFAULT 'UTC'")
            await conn.exec_driver_sql("ALTER TABLE users ADD COLUMN IF NOT EXISTS about_me TEXT NULL")
            await conn.exec_driver_sql("ALTER TABLE users ADD COLUMN IF NOT EXISTS morning_brief_at TIME NULL")
            await conn.exec_driver_sql("ALTER TABLE users ADD COLUMN IF NOT EXISTS evening_recap_at TIME NULL")
            await conn.exec_driver_sql(
                "ALTER TABLE users ADD COLUMN IF NOT EXISTS checkin_enabled BOOLEAN NOT NULL DEFAULT TRUE"
            )
            await conn.exec_driver_sql(
                "ALTER TABLE users "
                "ADD COLUMN IF NOT EXISTS knowledge_gap_data_consent BOOLEAN NOT NULL DEFAULT FALSE"
            )
            await conn.exec_driver_sql(
                "ALTER TABLE tasks "
                "ADD COLUMN IF NOT EXISTS updated_at "
