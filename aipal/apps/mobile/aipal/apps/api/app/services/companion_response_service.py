...
from ..llm_provider import active_provider_status, llm_chat, llm_chat_stream
...
async def generate_companion_response(
    ...
    *,
    ...
    log.info(
        "companion_response_context provider=%s model=%s fallback_active=%s fallback_reason=%s mode=%s context_sources=%s memory_items=%d history_items=%d",
        status.provider,
        status.model,
        status.fallback_active,
        status.fallback_reason,
        mode,
        sorted({str(item.get("source")) for item in context_items}),
        len([item for item in context_items if item.get("source") in {"memory", "recent_discussion", "history"}]),
    ...
    try:
        ...
        reply = _fallback_reply(user_message, context_items, mode, user_preferences)
        output_mode = mode
        output_should_create_task = should_create_task
        suggested_actions = backend_actions
        memory_suggestions = backend_memory_suggestions
    if not reply:
        reply = _fallback_reply(user_message, context_items, output_mode, user_preferences)
    return {
        "reply": reply,
        "mode": output_mode,
        "emotion": emotion,
...
def _fallback_reply(
    ...
    name = (
        str(preferences.get("wake_name") or preferences.get("display_name") or preferences.get("preferred_name") or preferences.get("first_name") or preferences.get("profile_name") or "")
        .strip()
    )
    if name.lower() in {"friend", "user", "there"}:
        name = ""
    greeting = f"{name}, " if name else ""

    if context_items:
        top_context = "; ".join(
            f"{item.get('source', 'context')}: {item['text']}"
            for item in context_items[:3]
            if item.get("text")
        )
        if top_context:
            if mode == "planner":
                return f"{greeting}I can still build a practical plan from the context I have: {top_context}. Start with the time-sensitive item first, then the highest-impact work."
            return f"{greeting}I’m staying with the thread and using the relevant context I found: {top_context}. The next useful step is the item with the nearest deadline or clearest impact."
    lower = user_message.lower().strip()
    if any(word in lower for word in ("hello", "hi", "hey")):
        return f"{greeting}I’m here and ready to help. What feels most important right now?"
    if mode in {"companion", "reflection"}:
        return f"{greeting}I’m still with you. Share the specific person, project, task, or goal and I’ll help from the context already available."
    return f"{greeting}I’m still with you. Give me the specific action or item and I’ll help from the context we already have."
