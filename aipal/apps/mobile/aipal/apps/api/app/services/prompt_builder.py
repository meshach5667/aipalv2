    ...
    def build(self, request: PromptBuildRequest) -> list[dict[str, str]]:
        ...
        if purpose == "reasoning":
            ...
            response_contract = (
                ...
                f"{schema}\n"
                "- Use only audit-safe fields: intent, required context, required tools, assumptions, response plan, and confidence.\n"
                "- Never expose chain-of-thought or private step-by-step reasoning."
            )
        system = (
            ...
            "- Understand the user's actual request first; answer it directly before suggesting action.\n"
            "- Ask a clarifying question only when a required fact is missing and no useful partial answer is possible.\n"
            "- When context exists, use relevant details naturally; never invent missing personal data.\n"
            "- Use the user's name naturally when it is available and relevant.\n"
            "- Separate known facts, assumptions, and suggestions when that distinction matters.\n"
            "- Prefer practical next actions over generic reassurance or motivational language.\n"
            "- Keep answers concrete, specific, and grounded in the current request and available context.\n"
            "- Avoid repeating context the user already knows unless it changes the recommendation.\n"
