...
from ..llm_provider import active_provider_status
...
async def try_run_reasoned_turn(
    ...
    *,
    ...
    log.info(
        "companion_intelligence_context provider=%s model=%s fallback_active=%s fallback_reason=%s context_sources=%s memory_items=%d query_results=%s candidate_count=%s",
        status.provider,
        status.model,
        status.fallback_active,
        status.fallback_reason,
        sorted({str(item.get("source")) for item in context_items}),
        len([item for item in context_items if item.get("source") in {"memory", "recent_discussion"}]),
        merged.get("metrics", {}).get("result_count"),
        merged.get("metrics", {}).get("candidate_count"),
    )
    ...
    log.info(
        "reasoning_validated intent=%s required_context=%s tools=%s assumptions=%s confidence=%.2f pending_resolution=%s errors=%d",
        outcome.decision.primary_intent,
        outcome.decision.required_context,
        [call.name for call in outcome.decision.tool_calls],
        outcome.decision.assumptions,
    ...
    log.info(
        "companion_final_response status=completed intent=%s mode=%s tools=%s validation_errors=%d total_ms=%d",
        outcome.decision.primary_intent,
