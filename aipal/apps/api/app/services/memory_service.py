...
def _build_candidate(
    ...
    *,
    ...
    if memory_type == "important_event":
        ...
        importance = 7
        follow_up_at = datetime.now(UTC) + timedelta(days=7)
        follow_up_prompt = f"What did that milestone lead to?"
...
async def search_memories(
    ...
    *,
    ...
    result = await db.execute(select(Memory).where(Memory.user_id == user_id, _memory_visibility_condition(include_pending=False)))
