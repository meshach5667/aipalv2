...
async def test_text_turn_prompt_filters_memories_and_caps_context():
    ...
    try:
        ...
        context_lines = [line for line in user_prompt.splitlines() if line.startswith("- summarized ")]
        assert "Life context narrative:" in user_prompt
...
def test_websocket_text_turn_uses_generate_companion_response():
    ...
    with (
        ...
        patch("app.services.companion_orchestrator.generate_companion_response_stream") as mock_generate,
        _no_tts_stream(),
    ):
        mock_rl.allow.return_value = True

        async def _stream(*_args, **_kwargs):
            yield {"type": "context_ready", "metrics": {"context_items_count": 0}}
            yield {"type": "reply_delta", "text": "Hmm, I am with you.", "metrics": {"first_token_ms": 1}}
            yield {"type": "sentence_ready", "text": "Hmm, I am with you."}
            yield {
                "type": "turn_complete",
                **_generate_payload("Hmm, I am with you."),
                "metrics": {"first_token_ms": 1},
            }

        mock_generate.side_effect = _stream
